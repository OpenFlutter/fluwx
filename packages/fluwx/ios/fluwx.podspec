...
The capability of implementing WeChat SDKs in Flutter. With Fluwx, developers can use WeChatSDK easily, such as sharing, payment, lanuch mini program and etc.
  ...
  s.resource_bundles = {'fluwx_privacy' => ['fluwx/Sources/fluwx/Resources/PrivacyInfo.xcprivacy']}
  ...
  s.subspec 'pay' do |sp|
    ...
    pod_target_xcconfig["GCC_PREPROCESSOR_DEFINITIONS"] = "$(inherited) FLUWX_WITH_PAY=1 #{logging_status}"
  ...
  s.subspec 'no_pay' do |sp|
    ...
    pod_target_xcconfig["GCC_PREPROCESSOR_DEFINITIONS"] = "$(inherited) FLUWX_NO_PAY=1 #{logging_status}"
    sp.pod_target_xcconfig = pod_target_xcconfig
  end
